  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!map.getSource(SOURCE_ID)) {
        map.addSource(SOURCE_ID, { type: 'geojson', data: hotlipsData as GeoJSON.FeatureCollection });
      }
      if (!map.getLayer(LAYER_ID)) {
        map.addLayer({
          id: LAYER_ID, type: 'symbol', source: SOURCE_ID,
          layout: {
            'icon-image': 'event-hotlips',
            'icon-size': ['interpolate', ['linear'], ['zoom'], 4, 0.4, 8, 0.6, 12, 0.8, 16, 1.0],
            'icon-allow-overlap': true,
            'visibility': hotLips.layerVisible ? 'visible' : 'none',
          },
        });
