...
export default function EventDetailCard({ event, onClose }: EventDetailCardProps) {
  ...
  useEffect(() => {
    ...
    // Pieni viive, jotta ei sulje heti avatessa
  ...
  const getCardStyle = () => {
    if (event.screenPosition) {
      ...
      // Varmista että kortti mahtuu näytölle
      let left = x - cardWidth / 2;
      let top = y - cardHeight - 20; // Avautuu ylöspäin

      // Jos ei mahdu ylös, avaa alas
      if (top < padding) {
        top = y + 20;
      }
